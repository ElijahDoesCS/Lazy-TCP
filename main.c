...
int parse_packet(bool print, Raw_Header *raw_packet, Virtual_Device *vd) {
    char *packet = raw_packet->bytes;
    int pack_len = raw_packet->pack_len;

    IPv4_Header *ip_pack = (IPv4_Header *) packet;
    int op = packet_deserialize(ip_pack), ihl = ip_pack->version_ihl & 0xf;
    ...
    switch (op) {   
        case IPV4_ICMP: // ICMP over IPv4
            ...
            icmp_dispatch(ip_pack, icmp_pack); // Dispactch to the ICMP module to handle the type/code
...
int main(int argc, char **argv) {
    ...
    char *tu = calloc(MTU_SIZE + 1, sizeof(char));
    if (tu == NULL) {
    ...
    while ((nread = read(vd.fd, tu, MTU_SIZE)) > 0) {
        ...
        if (parse_packet(verbose, &packet, &vd)) {
            virtual_device_destroy(vd);
            free(tu);
            exit(EXIT_FAILURE);
        }
